...
function myButton(){//button object
	...
	this.cue=1;
	this.loop=false;
    this.begining=0;//for continue
    this.content="";
    this.order;
    this.start=0;//seek result
    this.duration=0;
}
    ...
    for(let i=0;i<num1;i++){
		...
		let buttons_line='<div id="b-'+i+'" class="button">';
                ...
                buttons_line+='<div id="bnd-'+i+'" class="bn_div">';
					...
					//buttons_line+=b_num.padStart(2,"0");
...
function prepare_button(num){
	...
	//console.log("prepare_button t_ing");
	//t_ing(num);//t_ingでは自動停止の時に0.0にならない
...
function drop(e){
    ...
    if(drag_ele!=""){//データのウィンドウ外からのD&Dは=undef
        if(drag_ele.id=="tool_bar"){
            ...
            let drop_func=()=>{
                ...
                if(drop_files){
					...
					for(let i=0;i<repeat_count;i++){
                        if(drop_files[i]){
                            if(drop_files[i].type.indexOf("audio")>=0){// only sound data
...
function to_end(num){
    console.log("to_end in cancelAnimationFrame(button_array["+num+"].timer)");
	cancelAnimationFrame(button_array[num].timer);
    //clearInterval(button_array[num].timer)//function name
    //console.log("clear button_array[num].timer-ID="+button_array[num].timer);
    debi("mov-"+num).pause();
	console.log("button_array["+num+"].cue="+button_array[num].cue)
    if(button_array[num].cue==1){
        debi("mov-"+num).currentTime=0+button_array[num].start;
    }
...
/*sound fade out*/
var out_time=0;
function sound_fade_out(num,stamp_sa){
	out_time+=(stamp_sa/1000);
...
function start_to_play(num,fade){
	...
	let t_ing=(t_stamp)=>{
		...
		//if(running>=0){
			...
			debi("bti-"+num).innerHTML=running+"/";
			button_array[num].timer=requestAnimationFrame(t_ing);
			if(bf_t_stamp!=0){
				let stamp_sa=t_stamp-bf_t_stamp;
				stamp_cum+=stamp_sa;
				//console.log(stamp_cum);
				if(stamp_cum>1000){
					if(button_array[num].fading==1){
						sound_fade_out(num,stamp_sa);
					}
					if(button_array[num].fading==2){
						sound_fade_in(num,stamp_sa);
					}
					bf_t_stamp=0;
					stamp_cum=0;
				}
    ...
    //button_array[num].timer=setInterval(()=>{t_ing(num)},10);
	console.log("button_array["+num+"].timer requestAnimationFrame(t_ing)");
    button_array[num].timer=requestAnimationFrame(t_ing);
    let hue_num=button_array[num].color;
...
/*sound fade in*/
var in_time=0;
function sound_fade_in(num,stamp_sa){
	in_time+=(stamp_sa/1000);
	//in_time+=fade_time_get(t_stamp);
